# Telemetry setup (Azure Application Insights) — DEPRECATED
...
| Code | `src/service/telemetryService.ts` (currently a no-op stub) |
| Event inventory | `telemetry.json` (root) |

Telemetry only sends data when a non-empty connection string is configured in `telemetryService.ts`.
...
1. Restore the `@vscode/extension-telemetry` dependency: `npm i @vscode/extension-telemetry`
2. Re-implement the reporter in `src/service/telemetryService.ts` (see upstream [cweijan/vscode-office](https://github.com/cweijan/vscode-office) for the original implementation) and paste **your** connection string:
...
If telemetry is re-enabled, keep honoring these controls:

| Control | Setting |
|---------|---------|
| VS Code global | `telemetry.telemetryLevel` / `telemetry.enableTelemetry` |
| Extension only | `vscode-office.enableTelemetry` (contribution removed in this fork — re-add it together with the reporter) |
...
If `telemetry.json` is packaged with the extension, its events appear in that report.
...
| Still no events | VS Code telemetry off? Extension telemetry setting false? |
...
- `src/service/telemetryService.ts` — reporter and event API (currently a no-op stub)
